...
def get_usuario_actual(token: str = Depends(oauth2_scheme)) -> UsuarioActual:
    ...
    # Mapeamos las llaves en minúsculas que retorna psycopg2 al constructor de Pydantic
    return UsuarioActual(
        idUsu=fila["idUsu"],
        nombreUsu=fila["nombreUsu"],
        correoUsu=fila["correoUsu"],
...
def admin_eliminar_usuario(idUsu: str, admin: UsuarioActual = Depends(get_admin_usuario)):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                DELETE FROM rutina_ejercicio re
                USING rutina_plantilla rp
                WHERE re."idPlantilla" = rp."idPlantilla" AND rp."idUsu" = %s
            """, (idUsu,))
            
            # 2. Eliminar días de las rutinas del usuario
            cursor.execute("""
                DELETE FROM rutina_dias rd
                USING rutina_plantilla rp
                WHERE rd."idPlantilla" = rp."idPlantilla" AND rp."idUsu" = %s
            """, (idUsu,))
...
def buscar_ejercicios(q: str):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                'SELECT "idEjercicio", "nombreEj", tipo, equipamiento, nivel FROM ejercicios WHERE "nombreEj" ILIKE %s LIMIT 20',
                (f"%{q}%",)
            )
            return cursor.fetchall()
    finally:
...
def get_rutinas_por_fecha(idUsu: str, fecha: str):
    ...
    try:
        with connection.cursor() as cursor:
            # En Postgres se usa EXTRACT(ISODOW FROM CAST(%s AS DATE)) - 1 para que Lunes=0 ... Domingo=6
            cursor.execute(
                """
                SELECT 
                    rp."idPlantilla",
                ...
                WHERE rp."idUsu" = %s
                  AND CAST(%s AS DATE) BETWEEN rp.fecha_inicio AND rp.fecha_fin
                  AND rd.dia_semana = EXTRACT(ISODOW FROM CAST(%s AS DATE)) - 1
...
def _asegurar_tabla_medida(cursor) -> None:
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS medida_corporal (
            ...
            brazo_cm DOUBLE PRECISION,
            muslo_cm DOUBLE PRECISION,
            es_meta BOOL DEFAULT FALSE,
            CONSTRAINT uq_usuario_fecha UNIQUE ("idUsu", fecha)
        )
    """)
        ...
        with connection.cursor() as cursor:
            ...
            try:
                cursor.execute(
                    ...
                    JOIN rutina_dias rd ON rp."idPlantilla" = rd."idPlantilla"
                    WHERE rp."idUsu" = %s
                      AND rp.fecha_inicio <= CURRENT_DATE
                      AND rp.fecha_fin >= CURRENT_DATE
                    """,
                    (idUsu,),
                )
            ...
            try:
                ...
                rutinas_completadas_total = int((cursor.fetchone() or {}).get("total") or 0)
            except psycopg2.Error:
                pass
            ...
            try:
                ...
                rutinas_completadas_semana = int((cursor.fetchone() or {}).get("total") or 0)
            except psycopg2.Error:
                pass
...
def _asegurar_tablas_sesion(cursor) -> None:
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS sesion_entreno (
            ...
            fecha DATE NOT NULL,
            CONSTRAINT uq_sesion_usuario_rutina_fecha UNIQUE ("idUsu", "idPlantilla", fecha)
        )
    """)
    cursor.execute("""
    ...
    """)
    # En Postgres los índices se crean por separado
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_sesion ON sesion_ejercicio (id_sesion)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_ejercicio ON sesion_ejercicio ("idEjercicio")')
        ...
        with connection.cursor() as cursor:
            ...
            for f in cursor.fetchall():
                ...
                historial.append({
                    ...
                    "series": int(f["series"] or 0),
                    "repeticiones": int(f["repeticiones"] or 0),
                    "volumen": int(f["series"] or 0) * int(f["repeticiones"] or 0) * float(f["peso_kg"] or 0),
                })
            return historial
    except psycopg2.Error:
        return []
