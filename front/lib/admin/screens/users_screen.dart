...
class _UsersScreenState extends State<UsersScreen> {
  ...
  Widget build(BuildContext context){
    return Scaffold(appBar: AppBar(title: const Text('Usuarios')), body: FutureBuilder<List<Map<String, dynamic>>>(
      future: _usuarios, builder: (context, snapshot){
        ...
        return ListView.builder(
          ...
          itemBuilder: (ctx, i) {
            final u = usuarios[i];
            return ListTile(
              leading: CircleAvatar(child: Text((u['nombreUsu']??'?')[0].toUpperCase())),
              title: Text(u['nombreUsu']),
              subtitle: Text(u['correoUsu']),
              trailing: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  if(u['esAdmin']==1) const Chip(label: Text('Admin'), backgroundColor: Color.fromARGB(255, 195, 94, 253)),
